...
if (
		...
		|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'cron_executeall' )
) {

	wp_nonce_ays( 'cron_executeall' );
...
if(get_option('xyz_em_active') == 1){
	...
	else
	{
		...
		foreach( $xyz_em_campDetailsAll as $entry ) {
			...
			$counter=1;$xyz_em_fieldInfoDefaultValue='';
			...
			if(count($xyz_em_mappingDetails)>0){
				...
				foreach ($xyz_em_mappingDetails as $mappingdetail){
					...
					if(count($xyz_em_fieldValueDetails)> 0){
						...
						$xyz_em_subject = str_replace("{field1}",$xyz_em_fieldInfoDefaultValue,$xyz_em_subject);
					}
