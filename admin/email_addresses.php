...
if(isset($_GET['emailmsg'])){
	$xyz_em_emailMessage = intval($_GET['emailmsg']);
}
	...
	<h2>List Emails</h2>
		...
		if($_POST){
			...
			if (
					...
					|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'xyz_em_status' )
			) {
			
				wp_nonce_ays( 'xyz_em_status' );
		...
		{
			$xyz_em_email_address_status=intval($_REQUEST['xyz_em_email_address_status']);
...
<?php wp_nonce_field( 'xyz_em_status' );?>
...
<tbody>
        <?php if( count($entries)>0 ) { 
			...
			<?php 
		 $bare_url=admin_url('admin.php?page=newsletter-manager-manage-emails&action=email_unsubscribe&id='.$entry->id.'&pageno='.$pageno);
			
			if(($entry->status != 0) && ($entry->status != -1)){			
			?>
			<a
				href='<?php echo wp_nonce_url( $bare_url, 'email_unsubscribe_'.$entry->id ); ?>'
			...
			<?php 
				...
				$bare_url=admin_url('admin.php?page=newsletter-manager-manage-emails&action=email_activate&id='.$entry->id.'&pageno='.$pageno);
				
			?>
			<a
				href='<?php echo wp_nonce_url( $bare_url, 'email_activate_'.$entry->id ); ?>'
			...
			<?php $bare_url=admin_url('admin.php?page=newsletter-manager-manage-emails&action=email_delete&id='.$entry->id.'&pageno='.$pageno);
			?>
			<td id="tdCenter" ><a
			href='<?php echo wp_nonce_url( $bare_url, 'trash-email_'.$entry->id ); ?>'
