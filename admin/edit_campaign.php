...
if (
		...
		|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'edit-campaign_'.$xyz_em_campId )
) {

	wp_nonce_ays( 'edit-campaign_'.$xyz_em_campId );
...
// 		die;
	...
	$email_type = intval($_POST['email_type']);
	...
	$xyz_em_body=xyz_em_string_To_Emoji($xyz_em_body,$xyz_em_emojis);
	$xyz_em_senderEmail = sanitize_text_field($_POST['xyz_em_senderEmail']);
		$xyz_em_senderEmailFlag = 0;
		
		if(get_option('xyz_em_sendViaSmtp') == 1){
			$xyz_em_senderEmailId =sanitize_text_field( $_POST['xyz_em_senderEmailId']);
	...
	if (($_POST['xyz_em_campName']!= "")
			...
			&& ($xyz_em_senderEmailFlag == 0)){
				...
				$xyz_em_campName = sanitize_text_field($_POST['xyz_em_campName']);



				$xyz_em_startTime = sanitize_text_field($_POST['xyz_em_startTime']);
		$xyz_em_hour = intval($_POST['xyz_em_hour']);
		$xyz_em_minute =intval($_POST['xyz_em_minute']);
		$xyz_em_second = 00;
		
		$endTimeCondition = intval($_POST['triggerEndTimeCondition']);
		
		$xyz_em_endTime = sanitize_text_field($_POST['xyz_em_endTime']);
		$xyz_em_hour_end = intval($_POST['xyz_em_hour_end']);
		$xyz_em_minute_end = intval($_POST['xyz_em_minute_end']);
		$xyz_em_second_end = 00;

		$xyz_em_campSubject =sanitize_text_field($_POST['xyz_em_campSubject']);
		
		$xyz_em_batchSize = abs(intval($_POST['xyz_em_batchSize']));
		$xyz_em_senderName =sanitize_text_field($_POST['xyz_em_senderName']);
		$xyz_em_redirectAfterLink = strip_tags($_POST['xyz_em_redirectAfterLink']);
		$xyz_em_senderEmail =sanitize_text_field($_POST['xyz_em_senderEmail']);
		$xyz_em_join_after_campaign_started = intval($_POST['xyz_em_join_after_campaign_started']);
			...
			if($xyz_em_senderEmailFlag != 1){
				...
				if(count($xyz_em_campaign_count) == 0){
					...
					if($xyz_em_campFlag == 1){
						...
						?><script>
						document.location.href="admin.php?page=newsletter-manager-manage-campaigns&campmsg=2&pagenum=<?php echo $xyz_em_pagenum ?>";
						</script>
						<?php
						exit();
...
<div>
	...
	<form method="post" enctype="multipart/form-data">
		<?php wp_nonce_field( 'edit-campaign_'.$xyz_em_campId );?>
		<table class="widefat" style="width:98%;">
			...
			<tr>
				...
				<td><input readonly="readonly" name="xyz_em_startTime"
					...
					value="<?php if(isset($_POST['xyz_em_startTime']) && $_POST['xyz_em_startTime'] != ""){echo esc_attr($_POST['xyz_em_startTime']);}else{ echo esc_attr(xyz_local_date_time("d/m/Y",$details->start_time));}?>" />
					 <select
				...
				<td id="alignText">
					<div>
						...
						<div id="triggerEndTimeConditionDate"  >
							...
							value="<?php if(isset($_POST['xyz_em_endTime']) && $_POST['xyz_em_endTime'] != ""){echo esc_attr($_POST['xyz_em_endTime']);}else{if($details->end_time > 0){ echo esc_attr(xyz_local_date_time("d/m/Y",$details->end_time));}}?>" />	
							</div>
							<div >
				...
				<td >
					<div id="htmlcamp">
						...
						<?php 
							...
							if(isset($_POST['xyz_em_body'])){
								wp_editor($_POST['xyz_em_body'],'xyz_em_body');
							}else{
								wp_editor($details->body,'xyz_em_body');
							}
						}
			...
			<tr valign="top">
				...
				<td><input id="input" name="xyz_em_batchSize" type="text"
					id="xyz_em_batchSize"
					value="<?php if(isset($_POST['xyz_em_batchSize']) ) echo abs(intval($_POST['xyz_em_batchSize']));else echo absint($details->batch_size); ?>" />
				</td>
			</tr>
			<tr valign="top">
			...
			<tr valign="top">
				...
				<td><input id="input" name="xyz_em_redirectAfterLink" type="text"
					...
					value="<?php if(isset($_POST['xyz_em_redirectAfterLink']) ) echo strip_tags($_POST['xyz_em_redirectAfterLink']);else echo strip_tags($details->unsubscription_link); ?>" />
				</td>
			</tr>
			<?php 
			if(get_option('xyz_em_sendViaSmtp') == 1){
			...
			<tr valign="top">
				...
				?>
				<option value="<?php echo $xyz_em_getSender->id;?>" <?php if($details->sender_email_id==$xyz_em_getSender->id){?>selected="selected"<?php }?>><?php echo sanitize_text_field($xyz_em_getSender->user);?></option>
				<?php 
				}
				?>
				</select>
			...
			<tr>
				...
				<td>
							...
							$bare_url=admin_url('admin.php?page=newsletter-manager-manage-campaigns&action=attachment_delete&id='.$attachmentDetails->id.'&campId='.$details->id);
							
							?>
							<a
							href='<?php echo wp_nonce_url( $bare_url, 'attachment_delete' ); ?>'
								>Delete</a>
