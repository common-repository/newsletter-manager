...
if(isset($_POST['exportForm'])=="exportForm" && $execFlag == 1){
	if (
			...
			|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'xyz_em_formExport_nonce' )
			) {
				
				wp_nonce_ays( 'xyz_em_formExport_nonce' );
				
				exit();
				
			}
			$nameofForm = sanitize_text_field($_POST['exportForm']);
			...
			$xyz_em_status = intval($_POST['xyz_em_exportOption']);
			...
			if($xyz_em_status == 2)
				$exp_status_str=$wpdb->prepare("lm.el_id= %d",$xyz_em_listId);
					...
					if($completeExportFlag == 1){
						...
						//header("Location:".admin_url('admin.php?page=newsletter-manager-importexport&action=download&fileName='.$listName));
...
<?php wp_nonce_field( 'xyz_em_formExport_nonce');?>
	<input type="hidden" name="xyz_em_exportbatchSize" id="batchSize"
		...
		<input type="hidden" name="xyz_em_exportOption" value="<?php if(isset($_POST['xyz_em_exportOption'])) echo intval($_POST['xyz_em_exportOption']);else echo intval($xyz_em_status); ?>">
...
<?php
	...
	if(isset($_GET['exportNoActiveEmailmsg'])){
		$exportNoActiveEmailmsg = intval($_GET['exportNoActiveEmailmsg']);
	}
...
<?php 
	...
	if(isset($_POST['importForm'])=="importForm" && isset($_POST['xyz_em_batchSize']) || ($emptyError == 1)){
		if (
				! isset( $_REQUEST['_wpnonce'] )
				|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'xyz_em_formImport_nonce' )
		) {
		
			wp_nonce_ays( 'xyz_em_formImport_nonce' );
		...
		$formName = sanitize_text_field($_POST['importForm']);

		$xyz_em_emailListId = 1;
		$xyz_em_number_lines =absint( $_POST['xyz_em_batchSize']);

		if ($xyz_em_number_lines > 0){
			$xyz_em_separatorChar = sanitize_text_field($_POST['xyz_em_separator']);
			$xyz_em_enclosingChar = sanitize_text_field($_POST['xyz_em_enclCharacter']);
			$submited = 0;
			if(isset($_POST['submited'])){
				$submited = intval($_POST['submited']);
			}
			if($submited != 1){
				...
				$xyz_em_uploadFile_import = sanitize_text_field($_POST['xyz_em_uploadFile_import']);
			}
...
// 				echo "bat:".$xyz_em_batchCount."<br/>";
				if($xyz_em_currentBatch <= $xyz_em_batchCount){
					...
					if(isset($_POST['xyz_em_hiddenFtellValues'])){
						if($_POST['xyz_em_hiddenFtellValues'] != ""){
							fseek($file,intval($_POST['xyz_em_hiddenFtellValues']));
						}
					}
	...
	<?php wp_nonce_field( 'xyz_em_formImport_nonce' );?>
	...
	<?php
		...
		if(isset($_GET['importmsg'])){
			$importmsg =intval($_GET['importmsg']);
		}
