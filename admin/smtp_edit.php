...
if($_POST){
	$xyz_em_hiddenSmtpId = intval($_POST['xyz_em_hiddenSmtpId']);
	if (
			! isset( $_REQUEST['_wpnonce'] )
			|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'edit-smtp_'.$xyz_em_hiddenSmtpId )
	) {
	
		wp_nonce_ays( 'edit-smtp_'.$xyz_cfm_hiddenSmtpId );
...
if (($_POST['xyz_em_SmtpHostName']!= "") && ($_POST['xyz_em_SmtpEmailAddress'] != "") && ($_POST['xyz_em_SmtpPassword'] != "") && 
		($_POST['xyz_em_SmtpPortNumber']!= "") && ($_POST['xyz_em_SmtpSecuirity']!= "")){
			...
			$xyz_em_SmtpAuthentication = sanitize_text_field($_POST['xyz_em_SmtpAuthentication']);
			$xyz_em_SmtpHostName = sanitize_text_field($_POST['xyz_em_SmtpHostName']);
			$xyz_em_SmtpEmailAddress = sanitize_text_field($_POST['xyz_em_SmtpEmailAddress']);
			$xyz_em_SmtpPassword = $_POST['xyz_em_SmtpPassword'];
			$xyz_em_SmtpPortNumber = intval($_POST['xyz_em_SmtpPortNumber']);
			$xyz_em_SmtpSecuirity = sanitize_text_field($_POST['xyz_em_SmtpSecuirity']);
			$xyz_em_hiddenSmtpId = intval($_POST['xyz_em_hiddenSmtpId']);
			
			$blockedAccount_em = 0;
			
			if(sanitize_text_field($_POST['xyz_em_SmtpSetDefault'])=="on"){
...
<h2>Edit SMTP Account</h2>
	<form method="post">
		<?php wp_nonce_field( 'edit-smtp_'.$xyz_em_SmtpId );?>
	...
	<input type="hidden" name="xyz_em_hiddenSmtpId" value="<?php if(isset($_POST['xyz_em_hiddenSmtpId']) ){echo esc_attr($_POST['xyz_em_hiddenSmtpId']);}else{echo esc_attr($xyz_em_details->id);}?>"/>
</div>
</form>
</div>