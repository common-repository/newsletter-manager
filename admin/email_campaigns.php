...
var preview_campaign= '<?php echo wp_create_nonce('preview-campaign');?>';
          var dataString = { 
  				action: 'ajax_campaign_preview', 
  				id: id,
  				_wpnonce: preview_campaign
  		  };
			...
			<tr>
				<td colspan="18">
					<div style="float: right;">
						...
						<?php  $bare_url=admin_url('admin.php?page=newsletter-manager-manage-campaigns&action=cron&executeall=1');?>
						<a
							href='<?php echo wp_nonce_url( $bare_url, 'cron_executeall' ); ?>'><input
 ...
 -->
		<tbody>
			...
			<tr <?php echo $class; ?>>
				...
				if($entry->status == 1){
					$bare_url=admin_url('admin.php?page=newsletter-manager-manage-campaigns&action=campaign_status&id='.$entry->id.'&status=0&pageno='.$pageno);
					
					?>
				<a
				href='<?php echo wp_nonce_url( $bare_url, 'campaign_status_'.$entry->id ); ?>'><img
				...
				<?php 
					...
					$bare_url=admin_url('admin.php?page=newsletter-manager-manage-campaigns&action=campaign_status&id='.$entry->id.'&status=1&pageno='.$pageno);
				...
				<?php $bare_url=admin_url('admin.php?page=newsletter-manager-manage-campaigns&action=campaign_delete&id='.$entry->id.'&pageno='.$pageno);?>
				<td id="tdCenter"><a
					href='<?php echo wp_nonce_url( $bare_url, 'trash-campaign_'.$entry->id ); ?>'
				...
				if(($entry->status != 3) && ($entry->status != 2) && ($entry->status != -1)){
					$bare_url=admin_url('admin.php?page=newsletter-manager-manage-campaigns&action=send_mail&id='.$entry->id.'&pageno='.$pageno);
					?>
				<a
					href='<?php echo wp_nonce_url( $bare_url, 'send_mail_'.$entry->id ); ?>'><img
