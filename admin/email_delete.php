...
if (
		...
		|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'trash-email_'.$xyz_em_emailId )
) {
	wp_nonce_ays( 'trash-email_'.$xyz_em_emailId );
...
if(count($emailCount) == 0){
	...
	if($xyz_em_search=='')
		...
		header("Location:".admin_url('admin.php?page=newsletter-manager-searchemails&emailmsg=3'));
