...
if(isset($_POST['xyz_em_testMailId'])){
	if (
			...
			|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'test_mail')
	) {
	
		wp_nonce_ays( 'test_mail' );
	
		exit();
	
	} 
	if ($_POST['xyz_em_testMailId']!= ""){
		$xyz_em_testEmail = sanitize_text_field($_POST['xyz_em_testMailId']);
...
if(empty($xyz_em_campDetails)){
	...
	if(is_email($xyz_em_testEmail)){
      ...
      if($xyz_em_campDetails->sender_email_id == 0 || empty($xyz_em_Smtp_delete_check)){
...
if($sent == TRUE) {
			...
			?><script>
			document.location.href="admin.php?page=newsletter-manager-manage-campaigns&campmsg=4&pagenum=<?php echo $xyz_em_pageno ?>";
			</script>
			<?php
