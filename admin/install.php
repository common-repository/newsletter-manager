...
function em_install(){
	...
	if ($nlm_installed_date =="") {
		...
		update_option('nlm_installed_date', $nlm_installed_date);
	}
	...
	add_option('xyz_cfm_nlm_code',bin2hex(random_bytes(5)));//Charecter length 10 (in hexdec each byte is represented by two characters)
	add_option('xyz_em_widgetName','Subscribe');
	
	add_option('xyz_em_sendViaSmtp',0);
	add_option('xyz_em_SmtpDebug',0);
	
	add_option('xyz_em_captcha',0);
	/* recaptcha */
	
	    add_option("xyz_em_recaptcha_private_key",'');
        add_option("xyz_em_recaptcha_public_key",'');

		/* GDPR Settings */
	    add_option("xyz_em_sub_copyto_admin",'');
	    add_option("xyz_em_sub_screenshot_mail","");
	    add_option("xyz_em_sub_screenshot_mail_attachements","");
	    add_option("xyz_em_unsubscribe_copy_to_admin","");
		
	    if(get_option('xyz_em_credit_dismiss') == ""){
	        add_option("xyz_em_credit_dismiss",0);
	    }
	...
	$tblcolums = $wpdb->get_col("SHOW COLUMNS FROM  ".$wpdb->prefix."xyz_em_address_list_mapping");
	if(!in_array("formid", $tblcolums))
	    $wpdb->query("ALTER TABLE ".$wpdb->prefix."xyz_em_address_list_mapping ADD (`formid` int(20) COLLATE utf8_unicode_ci NOT NULL )");
	...
	if(in_array("track_count", $tblcolums))
	    $wpdb->query("ALTER TABLE ".$wpdb->prefix."xyz_em_email_campaign DROP `track_count`");
	...
	if(count($emailTemplateWelcomeCount) == 0){
	    ...
	    $wpdb->insert($wpdb->prefix.'xyz_em_email_template',array('id'=>1,'subject'=>"Subscription Active",'message'=>"<p>Hi {field1},</p>\r\n<p>Thank you for subscribing to our list.<br />\r\nYour subscription is active now.</p><p>You are currently subscribed to following list(s) <br /><i>{subscribed_lists}</i></p><p>Your subscription details are given below,</p>Email Address : {email-ID}<br />Date of subscription : {date}<br/>Time of subscription :{time}<br/>IP Address:{ip_address}<br />\r\n<p>Regards<br />\r\nYoursite.com<br />\r\n&nbsp;</p>"),
	        array('%d','%s','%s'));
	...
	if(count($emailTemplateWelcomeCount) == 0){
	    ...
	    $wpdb->insert($wpdb->prefix.'xyz_em_email_template',array('id'=>2,'subject'=>"Email Unsubscribed",'message'=>"<p>Hi {field1},</p>\r\n<p>Your email address has been successfully unsubscribed from our list.</p><p>Your email address has been unsubscribed from the  following list(s)</p><i>{unsubscribed_lists}</i><p>You are currently subscribed to following list(s) <br /><i>{subscribed_lists}</i></p><p>Your subscription details are given below,</p>Email Address : {email-ID}<br />Date of unsubscription : {date}<br/>Time of unsubscription :{time}<br/>IP Address:{ip_address}<br />\r\n<p>Regards<br />\r\nYoursite.com</p>"),
	        array('%d','%s','%s'));
	...
	if(count($emailTemplateWelcomeCount) == 0){
	    ...
	    $wpdb->insert($wpdb->prefix.'xyz_em_email_template',array('id'=>3,'subject'=>"Subscription Pending",'message'=>'<p>Hi {field1},</p><p>Thank you for subscribing to our list. <br />You are one click away from activating your subscription.<br />Just click the link below to  activate your subscription <br /><i><a href="{confirmation_url}">Confirm now</a></i></p><p>Regards<br />Yoursite.com</p>'),
	        array('%d','%s','%s'));
	
	}
	else
	{
	    $msg = $wpdb->get_results( 'SELECT message FROM '.$wpdb->prefix.'xyz_em_email_template WHERE id=3') ;
	    $msg=$msg[0];
	    
	    $msg= str_replace('{confirmation_link}','{confirmation_url}',$msg->message);
	    $wpdb->update($wpdb->prefix.'xyz_em_email_template', array('message'=>$msg), array('id'=>3));
	    
	}
	
	$emailTemplateWelcomeCount = $wpdb->get_results( 'SELECT subject FROM '.$wpdb->prefix.'xyz_em_email_template WHERE id=4' ) ;

	if(count($emailTemplateWelcomeCount) == 0){
	    
	    $wpdb->insert($wpdb->prefix.'xyz_em_email_template',array('id'=>4,'subject'=>" Update Your Subscription Status",'message'=>'<p>Hi {field1},</p><p>You have recently requested for updating  your subscription status. <br />Please click below  link to update your subscription status <br /><i><a href="{update_sub_status_url}">Update Now</a></i></p><p>You can ignore this message if this request not made by you.</p><p>Regards<br />Yoursite.com</p>'),
	        array('%d','%s','%s'));
