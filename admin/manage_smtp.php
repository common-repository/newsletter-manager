...
if(isset($_GET['smtpmsg'])){
	$smtpMessage = intval($_GET['smtpmsg']);
}
if($smtpMessage == 1){
...
<h2>SMTP Account List</h2>
	<form method="post">
			...
			<table class="widefat" style="width: 99%; margin: 0 auto">
				...
				if($entry->status == 0){
					$bare_url=admin_url('admin.php?page=newsletter-manager-manage-smtp&action=smtp-block&id='.$entry->id.'&status=1&pageno='.$pageno);
					?>	
						
					
						<td style="text-align: center;"><a
						href='<?php echo wp_nonce_url( $bare_url, 'smtp-status_'.$entry->id ); ?>'><img
				...
				<?php 
					...
					$bare_url=admin_url('admin.php?page=newsletter-manager-manage-smtp&action=smtp-activate&id='.$entry->id.'&status=0&pageno='.$pageno);
				...
				?>
						...
						<?php $bare_url=admin_url('admin.php?page=newsletter-manager-manage-smtp&action=smtp-delete&id='.$entry->id.'&pageno='.$pageno);
						 ?>
						<td style="text-align: center;" ><a
							href='<?php echo wp_nonce_url( $bare_url, 'trash-smtp_'.$entry->id ); ?>'
