...
if($_POST){
	if (
			...
			|| ! wp_verify_nonce( $_REQUEST['_wpnonce'],'create-campaign' )
	) {
		wp_nonce_ays( 'create-campaign');
		exit;
...
$xyz_em_senderEmailFlag = 0;
	...
	if(get_option('xyz_em_sendViaSmtp') == 1){
		$xyz_em_senderEmailId = intval($_POST['xyz_em_senderEmailId']);
			...
			if($xyz_em_senderEmailFlag != 1){
				...
				if(count($xyz_em_campaign_count) == 0){
					...
					if($xyz_em_campFlag == 1){
						...
						?><script>
						document.location.href="admin.php?page=newsletter-manager-manage-campaigns&campmsg=8";
						</script>
						<?php
				...
				<td id="alignText">
					<div>
						...
						<div id="triggerEndTimeConditionDate"  >
							...
							value="<?php if(isset($_POST['xyz_em_endTime']) && $_POST['xyz_em_endTime'] !="") echo esc_attr($_POST['xyz_em_endTime']); ?>" />
					...
					<div id="htmlcamp">
						...
						<?php 
							...
							if(isset($_POST['xyz_em_body']) && $_POST['xyz_em_body'] !=""){
								...
								$xyz_em_body = '';
								wp_editor($xyz_em_body,'xyz_em_body');
							}
