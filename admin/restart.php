...
if($_POST){
	...
	if (
			...
			|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'restart_'.$xyz_em_campId )
	) {
	
		wp_nonce_ays( 'restart_'.$xyz_cfm_hiddenSmtpId );
	...
	if ($_POST['campaignRestart']!= ""){
		...
		if($_POST['xyz_em_hiddenPageNumber'] == 0){
			...
			$xyz_em_hiddenPageNumber = absint($_POST['xyz_em_hiddenPageNumber']);
		}
...
<h2>Campaign Restart Option</h2>
	<form method="post">
	<?php wp_nonce_field( 'restart_'.$xyz_em_campId );?>
	...
	<input type="hidden" name="xyz_em_hiddenCampId" value="<?php if(isset($_POST['"xyz_em_hiddenCampId"']) ){echo esc_attr($_POST['"xyz_em_hiddenCampId"']);}else{echo esc_attr($xyz_em_campId);}?>"/>
	<input type="hidden" name="xyz_em_hiddenPageNumber" value="<?php if(isset($_POST['xyz_em_hiddenPageNumber']) ){echo esc_attr($_POST['xyz_em_hiddenPageNumber']);}else{echo esc_attr($xyz_em_pageno);}?>"/>
</div>
</form>
</div>
<?php 
